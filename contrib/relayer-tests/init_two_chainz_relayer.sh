#!/bin/bash
# init_two_chainz_relayer creates two stationwasmd chains and configures the relayer
...
# Ensure stationwasmd is installed
if ! [ -x "$(which stationwasmd)" ]; then
  echo "Error: wasmd is not installed. Try running 'make build-wasmd'" >&2
  exit 1
fi
...
# Stop existing stationwasmd processes
killall stationwasmd &> /dev/null
...
./one_chain.sh stationwasmd $chainid0 ./data 26657 26656 6060 9090
./one_chain.sh stationwasmd $chainid1 ./data 26557 26556 6061 9091
...
rly config init
rly config add-chains configs/stationwasmd/chains
rly config add-paths configs/stationwasmd/paths
